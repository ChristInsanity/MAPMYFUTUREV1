    ...
    <?php foreach ($jobs as $job): ?>
        <article class="card">
            <div class="flex justify-between gap-4 mb-4">
                ...
                <span class="badge text-blue-200 border-blue-500/30 bg-blue-500/10"><?= (int)$job['applicant_count'] ?> applicants</span>
            </div>
            <p class="text-slate-300 mb-4"><?= e($job['location'] ?: 'Location not set') ?> · <?= e(ucfirst($job['work_setup'])) ?> · <?= e($job['salary'] ?: 'Salary not posted') ?></p>
...
<div id="jobModal" class="hidden fixed inset-0 z-50 bg-black/70 px-4 py-8 overflow-y-auto">
    <div class="max-w-3xl mx-auto bg-[#162338] border border-[#334155] rounded-2xl p-6">
        ...
        <form id="jobForm" class="space-y-4">
            ...
            <button class="primaryBtn w-full" type="submit">Publish Job</button>
        </form>
    </div>
</div>
...
document.getElementById('openJobModal').addEventListener('click', () => jobModal.classList.remove('hidden'));
...
document.getElementById('jobForm').addEventListener('submit', async (event) => {
    ...
    if (result.success) {
        ...
        alert(result.message || 'Unable to create job.');
    }
});
</script>
